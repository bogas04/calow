...
describe("Link Length Comparison", () => {
  it("compares lengths across versions for a comprehensive meal", () => {
    ...
    console.log(`\nLink Length Comparison (Comprehensive Meal):`);
    console.log(`  V1: ${v1.length} chars`);
    console.log(`  V2: ${v2.length} chars`);
  ...
  it("compares lengths for a simple meal", () => {
    const simpleMeal = { ...COMPREHENSIVE_MEAL, items: [COMPREHENSIVE_MEAL.items[0]] };
    const v1 = createShareableMealLink(simpleMeal, BASE_LINK, "v1");
    const v2 = createShareableMealLink(simpleMeal, BASE_LINK, "v2");
    const v3 = createShareableMealLink(simpleMeal, BASE_LINK, "v3");

    console.log(`\nLink Length Comparison (Simple Meal):`);
    console.log(`  V1: ${v1.length} chars`);
    console.log(`  V2: ${v2.length} chars`);
    console.log(`  V3: ${v3.length} chars`);

    // Even for simple meals, V3 should be the winner
    assert.ok(v3.length < v2.length, "V3 should be shorter than V2");
  });
});
