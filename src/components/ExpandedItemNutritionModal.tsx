...
function ExpandedItemNutritionModal({
  ...
  hideWeight = false,
  onClose,
}: { item: ItemEntry; hideWeight?: boolean } & Pick<ModalProps, "isOpen" | "onClose">) {
        ...
        <ModalBody>
          ...
          {hasMicro && (
            <>
              ...
              {Object.entries(item.micro ?? {}).map(([microName, microValue]) => (
                <Flex key={microName} justify="space-between" align="center" my="1">
                  <Text textTransform="capitalize" fontWeight="400">
                    {microName}
                  </Text>
                  <Text>{Number((microValue * item.weight) / 100).toFixed(2)}</Text>
                </Flex>
              ))}
            </>
          )}
